...
import 'package:unpub/unpub_aws/lib/s3_file_store.dart';

void main() async {
  group('test s3 file store', () {
    late S3Store mockS3Store;

    setUp(() async {
      mockS3Store = S3Store('dart-pub-test',
        region: 'us-east-1',
        endpoint: 'localhost',
    ...
    // TODO: Test container credentials in aws_credentials

    test('upload-download-default-path', () async {
      await mockS3Store.upload('test_package', '1.0.0', testPackageData);
      var pkg1 =
          await readByteStream(mockS3Store.download('test_package', '1.0.0'));
      expect(pkg1, testPackageData);
    });

    test('upload-download-custom-path', () async {
      expect(
          mockS3Store.getObjectPath!.call('test_package', '1.0.0'),
          newFilePathFunc().call('test_package', '1.0.0'));
      expect(
          mockS3Store.getObjectPath!
              .call('test_package2', '2.0.0'),
          newFilePathFunc().call('test_package2', '2.0.0'));

      // A redundant test I suppose.  The upload method has a conditional baked in
      await mockS3Store.upload('test_package', '1.0.0', testPackageData2);
      var pkg2 = await readByteStream(
          mockS3Store.download('test_package', '1.0.0'));
      expect(pkg2, testPackageData2, reason: 'tar.gz content did not match');
    });

    test('require-default-aws-region', () async {
      var storePass = S3Store('dart-pub-test', region: 'us-east-1', credentials: mockCreds);
...
const testPackageData2 = [
  31, 139, 8, 0, 4, 81, 79, 97, 0, 3, 237, 207, 73, 10,
  128, 48, 12, 5, 80, 215, 158, 34, 39, 144, 84, 99, 123,
  30, 209, 138, 139, 98, 193, 214, 233, 246, 142, 116,
  37, 238, 84, 132, 188, 205, 39, 16, 194, 79, 163, 141,
  177, 137, 159, 124, 244, 28, 68, 148, 68, 176, 165, 82,
  98, 79, 76, 143, 249, 36, 65, 144, 72, 51, 153, 99, 38,
  17, 80, 144, 162, 60, 2, 124, 176, 83, 208, 59, 95, 116,
  107, 149, 210, 26, 237, 188, 30, 116, 235, 108, 123, 177,
  183, 174, 213, 245, 205, 157, 227, 17, 8, 249, 19, 141, 29,
  171, 57, 254, 186, 5, 99, 140, 177, 183, 45, 78, 193, 149,
  248, 0, 8, 0, 0];
